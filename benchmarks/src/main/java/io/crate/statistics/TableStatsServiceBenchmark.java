...
public class TableStatsServiceBenchmark {
    ...
    @State(Scope.Benchmark)
    public static class LoadFromDiskState {
        ...
        public void setup() throws IOException {
            tableStatsService = new TableStatsService(
                ...
                clusterService,
                mock(Sessions.class),
                Files.createTempDirectory("bench_load_table_stats")
            );
            Map<RelationName, Stats> stats = generateStats();
            tableStatsService.update(stats);
        }
    }

    @State(Scope.Benchmark)
    public static class SaveToDiskState {
        private TableStatsService tableStatsService;
        ...
        public void setup() throws IOException {
            tableStatsService = new TableStatsService(
                ...
                clusterService,
                mock(Sessions.class),
                Files.createTempDirectory("bench_save_table_stats")
            );
            stats = generateStats();
        }
    }
    ...
    @Benchmark
    public void measureLoadStatsFromDisk(LoadFromDiskState state, Blackhole blackhole) {
        for (int i = 0; i < 500; i++) {
            var relationName = new RelationName("schema_" + i, "table" + i);
            blackhole.consume(state.tableStatsService.loadFromDisk(relationName));
        }
    }

    @Benchmark
    public void measureSaveStatsFromDisk(SaveToDiskState state) {
        state.tableStatsService.update(state.stats);

    }

    private static Map<RelationName, Stats> generateStats() {
        Random random = new Random(System.currentTimeMillis());
        Map<RelationName, Stats> stats = HashMap.newHashMap(500);
        for (int i = 0; i < 500; i++) {
            var relationName = new RelationName("schema_" + i, "table" + i);
            long numDocs = i * 1000;
