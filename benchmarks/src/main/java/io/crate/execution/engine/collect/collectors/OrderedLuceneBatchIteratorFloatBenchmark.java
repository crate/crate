...
import static io.crate.execution.engine.sort.LuceneSort.nullAwareNumericSortField;
...
public class OrderedLuceneBatchIteratorFloatBenchmark {

    private static final TypedRowAccounting ROW_ACCOUNTING = new TypedRowAccounting(List.of(LongType.INSTANCE), RamAccounting.NO_ACCOUNTING);
    ...
    public void createLuceneBatchIterator() throws Exception {
        try (IndexWriter iw = new IndexWriter(
            ...
            System.out.println("No Nulls: " + noNulls);
            for (int i = 0; i < 10_000_000; i++) {
                Document doc = new Document();
                if (noNulls) {
                    doc.add(new FloatDocValuesField(columnName, i));
                } else {
                    if (i % 2 != 0) {
                        doc.add(new FloatDocValuesField(columnName, i));
                    }
    ...
    public void measureLoadAndConsumeOrderedLuceneBatchIterator(Blackhole blackhole) throws Exception {
        BatchIterator<Row> it = OrderedLuceneBatchIteratorFactory.newInstance(
            ...
            OrderingByPosition.rowOrdering(List.of(DataTypes.FLOAT), new int[]{0}, reverseFlags, nullsFirst),
            ROW_ACCOUNTING,
    ...
    private LuceneOrderedDocCollector createOrderedCollector(IndexSearcher searcher,
        ...
        // sortField.setMissingValue(NullSentinelValues.nullSentinelForScoreDoc(DataTypes.FLOAT, reverseFlags[0], nullsFirst[0]));
        //
        ...
        return new LuceneOrderedDocCollector(dummyShardId, searcher, MatchAllDocsQuery.INSTANCE, null, false, 10_000_000, RamAccounting.NO_ACCOUNTING, collectorContext, f -> null, new Sort(sortField), expressions, expressions);
    }
}
