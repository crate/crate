...
- Added support for CrateDB license management enabling users to trial the
  ...
  the community edition. Additionally, a new ``SET LICENSE`` statement has been
  added for license registration, and the ``license.ident`` setting has become
  ``@deprecated``.
...
.. _blogpost: https://cratedb.com/blog/cratedb-3-2-stable-available-now
