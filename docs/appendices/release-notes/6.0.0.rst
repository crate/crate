...
- Changed the output column names of a query result, which correspond to
 ...
 - :ref:`stddev` changed and is now an alias for :ref:`aggregation-stddev-samp`,
   to match PostgreSQL behavior. In order to calculate the population standard
   deviation, :ref:`aggregation-stddev-pop` should be used.
