...
- Improved the error message that is raised when a ``TEXT`` value exceeds the
  maximum length that can be indexed or stored in the column store. It now
  contains the name of the column instead of its internal numeric identifier
  (oid).
