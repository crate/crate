...
- Added requirement for :ref:`AL <privilege_types_al>` privilege on cluster
  ...
  * ``RESTORE SNAPSHOT ... USERMANAGEMENT``
  ...
  Previously the ``DDL`` privilege alone was sufficient. For more details see
  the `security advisory <https://github.com/crate/crate/security/advisories/GHSA-q2wh-fx2h-rqfh>`__.
...
- Authentication via HTTP is no longer cached on connection level to support
  proxy use-cases with keepalive. For more details see the
  `security advisory <https://github.com/crate/crate/security/advisories/GHSA-8jr8-9wjf-7m2h>`__.
...
- Limited the duration clients using the PostgreSQL wire protocol can remain
  ...
  `security advisory <https://github.com/crate/crate/security/advisories/GHSA-gvhg-w5hf-g282>`__.
...
- Limited the size of messages accepted from unauthenticated clients using the
  ...
  attacks. For more details see the
  `security advisory <https://github.com/crate/crate/security/advisories/GHSA-gvhg-w5hf-g282>`__.
...
- Fixed an issue that allowed authenticated users to bypass the ``COPY FROM``
  ...
  superusers. For more details see the
  `security advisory <https://github.com/crate/crate/security/advisories/GHSA-xw2x-w5xq-9w8r>`__.
...
- Fixed a security vulnerability that could allow clients to spoof the IP
  ...
  For more details see the
  `security advisory <https://github.com/crate/crate/security/advisories/GHSA-x296-hxfr-jq2h>`__.
...
- Fixed an issue that allowed authenticated users to read data from tables and
  ...
  For more details see the
  `security advisory <https://github.com/crate/crate/security/advisories/GHSA-cf3f-xgm5-jfrm>`__.

- Fixed incorrect results in :ref:`rank() <window-functions-rank>` and
  :ref:`dense_rank() <window-functions-dense-rank>` window functions when an
  explicit window frame is specified, e.g.::
...
- Fixed an issue where :ref:`lag <window-functions-lag>` and
  :ref:`lead <window-functions-lead>` ignored row-dependent offset and default
