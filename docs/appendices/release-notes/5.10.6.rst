...
- Fixed a bug where the session setting ``SET error_on_unknown_object_key = FALSE``
  was not persisted into a view, and such not taken into account when executing the view.
  Previously created views need to be recreated to apply the setting successfully.
