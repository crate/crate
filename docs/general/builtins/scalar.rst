...
``to_timestamp(expression, format_string)``
--------------------------------------

The ``to_timestamp`` function converts a textual representation of a timestamp
to an actual timestamp, based on a given format string. It is the inverted form
of the ``to_char`` function for timestamps.
...
``expression`` is a textual representation of a timestamp.

Format
......

The syntax for the format string follows that of the `Java DateTimeFormatter`_.
For reference, the table of permitted tokens is replicated below:
...
::
    ...
    cr> select to_timestamp('Thursday, January 1 - 5:31 PM 1970 AD', 'EEEE, LLLL d - h:m a uuuu G') as ts;
    +----------+
    | ts       |
    +----------+
    | 63060000 |
...
The ``to_date`` function converts a textual representation of a date
to an actual date, based on a given format string. It is the inverted form
of the ``to_char`` function for timestamps, without time information.
...
``expression`` is a textual representation of a date.
...
::
    ...
    cr> select to_date('Thursday, January 1 1970 AD', 'EEEE, LLLL d uuuu G') as ts;
    +----+
    | ts |
    +----+
    | 0  |
