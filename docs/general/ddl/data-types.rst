...
or comparing floating-point values for equality might not always be correct.
...
string representation. Internally ips are stored as ``bigint`` allowing
expected sorting, filtering and aggregation.
...
expressions <arithmetic>` that involves temporal :ref:`operands
<gloss-operand>`:
...
convertible to the desired data type, otherwise an error is thrown.
