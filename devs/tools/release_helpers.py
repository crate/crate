...
Used by ``update-rel-notes-and-version.py`` and ``bump-version-to-snapshot.py``,
which both prepare a single commit on a branch off ``origin/<major>.<minor>``
and print a link to open the pull request.
"""
...
def commit_and_push(root, branch, message, paths):
