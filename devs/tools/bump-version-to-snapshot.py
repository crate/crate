...
Given the next patch version like 6.5.1 it creates a ``bump-6.5.1`` branch off
``origin/6.5``, containing a single "Bump version to 6.5.1-SNAPSHOT" commit
which:

- sets the version in all ``pom.xml`` files to 6.5.1 by running
  ``./mvnw versions:set``

- adds a ``V_6_5_1`` constant with the snapshot flag set to
  ``server/src/main/java/org/elasticsearch/Version.java`` and makes it
  ``CURRENT``

- adds an "Unreleased" ``docs/appendices/release-notes/6.5.1.rst`` and lists it
...
Usage::
    ...
    ./devs/tools/bump-version-to-snapshot.py 6.5.1
"""
