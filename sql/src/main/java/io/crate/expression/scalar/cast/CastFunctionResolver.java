...
public class CastFunctionResolver {
    ...
    public static Symbol generateCastFunction(Symbol sourceSymbol, DataType targetType, boolean tryCast) {
        ...
        return new io.crate.expression.symbol.Function(functionInfo, ImmutableList.of(sourceSymbol));
    }

    public static boolean isCastFunction(Symbol symbol) {
        return symbol instanceof io.crate.expression.symbol.Function
               && ((io.crate.expression.symbol.Function) symbol).info().ident().name().startsWith(TO_PREFIX);
    }
