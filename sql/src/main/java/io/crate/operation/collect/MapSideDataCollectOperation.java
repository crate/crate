...
public class MapSideDataCollectOperation implements CollectOperation<Object[][]> {
    ...
    protected ListenableFuture<Object[][]> handleShardCollect(CollectNode collectNode, RamAccountingContext ramAccountingContext) {
        ...
        for (Map.Entry<String, Map<String, Set<Integer>>> nodeEntry : collectNode.routing().locations().entrySet()) {
            if (nodeEntry.getKey().equals(localNodeId)) {
                Map<String, Set<Integer>> shardIdMap = nodeEntry.getValue();
                ...
                for (Set<Integer> shardIdMap : nodeEntry.getValue().values()) {
