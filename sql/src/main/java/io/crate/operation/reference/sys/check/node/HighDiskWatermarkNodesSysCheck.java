...
public class HighDiskWatermarkNodesSysCheck extends DiskWatermarkNodesSysCheck {
    ...
    @Override
    protected boolean validate(FsInfo fsInfo, double diskHighWatermarkPercents, long diskHighWatermarkBytes) {
        for (FsInfo.Path path : fsInfo) {
            ...
            if ((usedDiskAsPercentage > diskHighWatermarkPercents)
                    || (path.getFree().bytes() < diskHighWatermarkBytes)) {
                return false;
            }
        }
        return true;
