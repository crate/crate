...
public class InsertStatementAnalyzer extends DataStatementAnalyzer<InsertAnalysis> {
    ...
    public Symbol visitInsert(Insert node, InsertAnalysis context) {
        ...
        if (node.columns().size() == 0) {
            ...
            for (ReferenceInfo columnInfo : context.table().columns()) {
                ...
                if (routing.equalsIgnoreCase(columnInfo.ident().columnIdent().fqn())) {
                    context.routingColumnIndex(impliedColumns.size());
                }
        ...
        if (!context.table().clusteredBy().equalsIgnoreCase("_id") && context.routingColumnIndex() < 0) {
    ...
    private Reference addColumn(ReferenceIdent ident, InsertAnalysis context) {
        ...
        // set routing if found
        String routing = context.table().clusteredBy();
        if (ident.columnIdent().fqn().equalsIgnoreCase(routing)) {
        ...
        for (Expression expression : node.values()) {
            ...
            try {
                ...
                if (context.primaryKeyColumnIndices().contains(i)) {
                    ...
                    int idx = context.table().primaryKey().indexOf(column.info().ident().columnIdent().name());
                    if (primaryKeyValues.size() > idx) {
                       primaryKeyValues.set(idx, value.toString());
                    } else {
