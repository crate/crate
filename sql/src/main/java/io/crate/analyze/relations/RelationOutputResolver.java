...
public class RelationOutputResolver {
    ...
    public RelationOutput getRelationOutput(QualifiedName qualifiedName, @Nullable List<String> path) {
        ...
        for (Map.Entry<QualifiedName, AnalyzedRelation> entry : sources.entrySet()) {
            ...
            assert sourceParts.size() == 2 : "QualifiedName in sources must have 2 parts: schemaName and tableName";
            String sourceSchema = sourceParts.get(0);
            String sourceTableOrAlias = sourceParts.get(1);
            AnalyzedRelation sourceRelation = entry.getValue();

            if (columnSchema != null && !sourceSchema.equals(columnSchema)) {
                continue;
            }
