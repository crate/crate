...
public class PlanOutputSymbolExtractor  {
    ...
    private static class Visitor extends PlanVisitor<Void, List<? extends Symbol>> {
        ...
        @Override
        public List<? extends Symbol> visitCollect(Collect plan, Void context) {
            return plan.collectPhase().toCollect();
        }

        @Override
        public List<? extends Symbol> visitMerge(Merge merge, Void context) {
            Optional<Projection> optFinalProjection = merge.mergePhase().finalProjection();
            if (optFinalProjection.isPresent()) {
                return optFinalProjection.get().outputs();
            }
            return process(merge.subPlan(), context);
        }
