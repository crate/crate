...
public class ExpressionAnalyzer {
    ...
    protected Symbol convertFunctionCall(FunctionCall node, ExpressionAnalysisContext context) {
        ...
        List<String> fName = node.getName().getParts();
        assert fName.size() <= 2 : "must be compliant with \"[schema.] function\" format";
        if (fName.size() == 1) {
            fName = Arrays.asList(null, fName.get(0));
        }
        ...
        if (node.isDistinct()) {
            ...
            String nodeName = "collection_" + fName.get(1);
            ...
            try {
                functionInfo = getFunctionInfo(fName.get(0), nodeName, outerArgumentTypes);
            ...
            functionInfo = getFunctionInfo(fName.get(0), fName.get(1), argumentTypes);
        }
