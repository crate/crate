...
public abstract class AbstractDataAnalysis extends Analysis {
    ...
    /**
     * Adds a computed id for given primary key values and/or clustered-by(routing) value
     *
     * @param primaryKeyValues
     * @param clusteredByValue
     */
    public void addId(List<String> primaryKeyValues, @Nullable String clusteredByValue) {
        addId(false, primaryKeyValues, clusteredByValue);
    }

    /**
     * Adds a computed id for given primary key values and/or clustered-by(routing) value
     * If the create flag is set and table allows auto-generation of id's, primary key values
     * are ignored.
     *
     * @param create
     * @param primaryKeyValues
     * @param clusteredByValue
     */
    protected void addId(Boolean create, List<String> primaryKeyValues, @Nullable String clusteredByValue) {
        String clusteredBy = table().clusteredBy();
        List<String> idParts = new ArrayList<>();
        boolean clusteredByAdded = false;
        if (table().hasAutoGeneratedPrimaryKey() && create) {
            idParts.add(Strings.randomBase64UUID());
        } else {
            if (table().primaryKey().size() != primaryKeyValues.size()) {
            ...
            for (int i=0; i<table().primaryKey().size(); i++)  {
                ...
                if (table.primaryKey().get(i).equalsIgnoreCase(clusteredBy)) {
        ...
        } else if (clusteredBy == null || clusteredBy.equalsIgnoreCase("_id")) {
            useIdForRouting = true;
        } else if (clusteredByValue == null) {
        ...
        String id = Joiner.on(Constants.ID_SEPARATOR).join(idParts);
        ids.add(id);
