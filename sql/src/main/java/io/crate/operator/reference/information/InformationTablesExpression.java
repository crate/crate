...
public abstract class InformationTablesExpression<T>
    ...
    public static final ImmutableList<InformationTablesExpression<?>> IMPLEMENTATIONS
            ...
            .add(new InformationTablesExpression<BytesRef>("schema_name") {
                ...
                public BytesRef value() {
                    return new BytesRef(Objects.firstNonNull(tableInfo.ident().schema(),
                            DocSchemaInfo.NAME));
                }
            })
            .add(new InformationTablesExpression<BytesRef>("table_name") {
                @Override
                public BytesRef value() {
                    return new BytesRef(tableInfo.ident().name());
                }
            })
            .add(new InformationTablesExpression<BytesRef>("clustered_by") {
                @Override
                public BytesRef value() {
                    return new BytesRef(tableInfo.clusteredBy());
                }
            })
            .build();
    ...
    protected InformationTablesExpression(ColumnIdent ident) {
        this.info = InformationSchemaInfo.TABLE_INFO_TABLES.getColumnInfo(ident);
        assert info != null: "no info for: " + ident;
    }
    ...
    @Override
    public void setNextRow(TableInfo row) {
        tableInfo = row;
    }
}
