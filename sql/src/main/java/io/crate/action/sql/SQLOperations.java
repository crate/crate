...
import io.crate.protocols.postgres.*;
...
public class SQLOperations {
    ...
    public class Session {
        ...
        public TransportKillJobsNodeAction getTransportKillJobsNodeAction() {
        ...
        private Portal getOrCreatePortal(String portalName, String statementName, Statement statement) {
            Portal portal = portals.get(portalName);
            if (portal == null) {
                portal = new SimplePortal(portalName, this);
                portals.put(portalName, portal);
            } else {
                if (statement.equals(portal.getStatement(statementName)) &&
                    Portal.PortalType.SIMPLE.equals(portal.type())) {
                    portal = new BulkPortal((SimplePortal)portal);
                    portals.put(portalName, portal);
                }
            }
            return portal;
        ...
        public void bind(String portalName,
            ...
            try {
                Portal portal = getOrCreatePortal(portalName, statementName, statement);

                portal.addQuery(statementName, query);
                portal.addStatement(statementName, statement);
                portal.addResultFormatCodes(resultFormatCodes);
                portal.addParams(params);
                if (portal.getAnalysis(statementName) == null) {
                    // analyze only once in a batch operation where there is
                    // Parse -> Bind -> Describe -> Execute -> Parse -> Bind -> Describe -> Execute -> ... -> Sync
                    portal.addAnalysis(statementName, analyzer.analyze(statement,
                                                                       new ParameterContext(portal.getArgs(),
                                                                                            EMPTY_BULK_ARGS,
                                                                                            defaultSchema)));
                }
            } catch (Throwable t) {
        ...
        public List<Field> describe(char type, String portalOrStatement) {
            ...
            try {
                ...
                Analysis analysis = portal.getLastAnalysis();
                if (analysis.rootRelation() == null) {
                    return null;
                }
                List<Field> fields = analysis.rootRelation().fields();
                portal.addOutputTypes(Symbols.extractTypes(fields));
                return fields;
            } catch (Throwable t) {
                throwable = t;
                throw t;
        ...
        public void execute(String portalName, int maxRows, ResultReceiver resultReceiver) {
            ...
            try {
                ...
                validateReadOnly(portal.getLastAnalysis());
                portal.addResultReceiver(resultReceiver);
                portal.setMaxRows(maxRows);
        ...
        public void sync(CompletionListener listener) {
            ...
            if (throwable == null) {
                ...
                portal.execute(analyzer, planner, statsTables, listener);
        ...
        public String query(String portalName) {
