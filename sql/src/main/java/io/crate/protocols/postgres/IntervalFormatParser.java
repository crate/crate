...
public class IntervalFormatParser {

    private IntervalFormatParser() {}
        ...
        try {
            ...
            for (int i = 1; st.hasMoreTokens(); i++) {
                ...
                if ((i & 1) == 1) {
                    ...
                    if (endMinutes != -1) {
                        seconds = getSeconds(token.substring(endMinutes + 1));
                        milliSeconds = getMilliSeconds(token.substring(endMinutes + 1));
                    }
                    ...
                    if (token.startsWith("year")) {
                        ...
                        seconds = getSeconds(valueToken);
                        milliSeconds = getMilliSeconds(valueToken);
                    }
                }
            }
    ...
    private static int getSeconds(String value) throws NumberFormatException {
        return new BigDecimal(value).intValue();
    }

    private static int getMilliSeconds(String value) throws NumberFormatException {
        return new BigDecimal(value).subtract(new BigDecimal(getSeconds(value))).multiply(new BigDecimal(1000)).intValue();
    }
}
