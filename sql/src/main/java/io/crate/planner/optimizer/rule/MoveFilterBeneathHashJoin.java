...
public final class MoveFilterBeneathHashJoin implements Rule<Filter> {
    ...
    public LogicalPlan apply(Filter filter, Captures captures) {
        ...
        Set<QualifiedName> leftName = hashJoin.lhs().baseTables().stream().map(AnalyzedRelation::getQualifiedName).collect(Collectors.toSet());
        Set<QualifiedName> rightName = hashJoin.rhs().baseTables().stream().map(AnalyzedRelation::getQualifiedName).collect(Collectors.toSet());
