    ...
    protected Plan visitCreateTableAnalysis(CreateTableAnalysis analysis, Context context) {
        ...
        if (analysis.isPartitioned()) {
            createTableNode = CreateTableNode.createPartitionedTableNode(
                    tableIdent.name(),
                    analysis.tableSettings().settings().getByPrefix("index."),
                    analysis.mapping(),
...
//                finalOutputs.add(new InputColumn(i, outputSymbols.get(i).valueType()));
     ...
     */
    public void optimizedReduceOnCollectorGroupBy(SelectAnalysis analysis, Plan plan, Context context) {
        ...
        if (havingClause != null) {
            FilterProjection fp = new FilterProjection((Function)havingClause);
            fp.outputs(contextBuilder.genInputColumns(groupProjection.outputs().size()));
    ...
    private void nonDistributedGroupBy(SelectAnalysis analysis, Plan plan, Context context) {
        ...
        if (havingClause != null) {
            FilterProjection fp = new FilterProjection((Function)havingClause);
            fp.outputs(contextBuilder.genInputColumns(handlerGroupProjection.outputs().size()));
            contextBuilder.addProjection(fp);
        }
        if (!ignoreSorting) {
            TopNProjection topN = new TopNProjection(
