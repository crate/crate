...
public class UserDefinedFunctionsIntegrationTest extends SQLTransportIntegrationTest {
    ...
    public void testCreateFunctionWithObjectInputType() throws Exception {
        execute("create function foo(object) returns long language javascript as 'function foo(x) { return x.foo; }'");
        execute("select foo(obj) from test where id = 1");
    ...
    public void testCreateOverloadedFunction() throws Exception {
        execute("create function foo(object) returns string language javascript as 'function foo(x) { return \"1\"; }'");
        execute("create function foo(string) returns string language javascript as 'function foo(x) { return x; }'");
