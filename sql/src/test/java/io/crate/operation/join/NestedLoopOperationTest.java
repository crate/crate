...
public class NestedLoopOperationTest extends RandomizedTest {
    ...
    private class ImmediateTestTask extends Task<QueryResult> {
        ...
        public ImmediateTestTask(Object[][] rows, int limit, int offset) {
            ...
            this.result = ImmutableList.of(
                    Futures.immediateFuture(new QueryResult(limitedRows)));
        }
    ...
    private class TestDQLNode extends AbstractDQLPlanNode {
        ...
        public Task<QueryResult> task() {
            return new ImmediateTestTask(rows, limit(), offset());
        }
    }
    ...
    private class TestExecutor implements Executor {
        ...
        public Job newJob(Plan node) {
            Job job = new Job();
            if (!node.isEmpty()) {
                job.addTask(newTask(node.iterator().next(), job.id()));
            }
            return job;
        }
        ...
        public Task newTask(PlanNode planNode, UUID jobId) {
            return ((TestDQLNode)planNode).task();
        }
