...
public class AnyLikeOperatorTest {
    ...
    private static Symbol normalizeSymbol(String pattern, String ... expressions) {
        ...
        Literal valuesLiteral = Literal.newLiteral(new ArrayType(StringType.INSTANCE), expressions);
        FunctionImplementation<Function> impl = new AnyLikeOperator().getForTypes(
                Arrays.asList(valuesLiteral.valueType(), patternLiteral.valueType())
        );
        AnyLikeOperator op = new AnyLikeOperator(impl.info());
    ...
    private Boolean anyLike(String pattern, String ... expressions) {
        Literal patternLiteral = Literal.newLiteral(pattern);
        Literal valuesLiteral = Literal.newLiteral(new ArrayType(DataTypes.STRING), expressions);
        FunctionImplementation<Function> impl = new AnyLikeOperator().getForTypes(
                Arrays.asList(valuesLiteral.valueType(), DataTypes.STRING)
        );
        AnyLikeOperator op = new AnyLikeOperator(impl.info());
        return op.evaluate(valuesLiteral, patternLiteral);
    }
