...
public class SelectStatementAnalyzerTest extends CrateUnitTest {
    ...
    public void testUnionAsSubSelectRewritten() throws Exception {
        SelectAnalyzedStatement analysis = analyze("select id, text from (" +
                                                     "select id, name, text from (" +
                                                        "select * from users) a where id = 1 " +
                                                     "union all " +
                                                     "select id, name, name as text from (" +
                                                        "select * from users_multi_pk) a where id = 2 " +
                                                     "order by id) a " +
        ...
        assertThat(tableUnion.first().querySpec(), isSQL("SELECT doc.users.id, doc.users.text " +
                                                         ...
                                                         "LIMIT add(5, 5)"));
        assertThat(tableUnion.second().querySpec(), isSQL("SELECT doc.users_multi_pk.id, doc.users_multi_pk.name " +
                                                          "WHERE (((doc.users_multi_pk.id = 1) OR " +
                                                                "(doc.users_multi_pk.name = 'foo')) " +
    ...
    public void testUnionAsSubSelectNotRewritten() throws Exception {
        SelectAnalyzedStatement analysis = analyze("select id, text from (" +
                                                    "select id, name, text from users where id = 1 " +
                                                    "union all " +
                                                    "select id, name, name as text from users_multi_pk where id = 2 " +
