...
public class CreateAlterPartitionedTableAnalyzerTest extends BaseAnalyzerTest {
    ...
    public void testAlterPartitionedTable() throws Exception {
        ...
        assertEquals("0-all", analysis.tableSettings().settings().get(TableSettingsInfo.AUTO_EXPAND_REPLICAS));
    }
    ...
    public void testAlterPartitionedTablePartition() throws Exception {
        ...
        assertThat(analysis.table().tableSettingsInfo(), instanceOf(AlterPartitionedTableSettingsInfo.class));
        AlterPartitionedTableSettingsInfo tableSettingsInfo = (AlterPartitionedTableSettingsInfo)analysis.table().tableSettingsInfo();
        assertThat(tableSettingsInfo.partitionTableSettingsInfo(), instanceOf(TableSettingsInfo.class));
        assertEquals("0-all", analysis.tableSettings().settings().get(TableSettingsInfo.AUTO_EXPAND_REPLICAS));
    }
    ...
    public void testAlterPartitionedTableShards() throws Exception {
        ...
        assertThat(analysis.table().isPartitioned(), is(true));
        assertThat(analysis.table().tableSettingsInfo(), instanceOf(AlterPartitionedTableSettingsInfo.class));
        assertEquals("10", analysis.tableSettings().settings().get(TableSettingsInfo.NUMBER_OF_SHARDS));

        AlterPartitionedTableSettingsInfo tableSettingsInfo = (AlterPartitionedTableSettingsInfo)analysis.table().tableSettingsInfo();
        TableSettings tableSettings = new TableSettings(
                analysis.tableSettings().settings(),
                tableSettingsInfo.partitionTableSettingsInfo().supportedInternalSettings());
        assertEquals(null, tableSettings.settings().get(TableSettingsInfo.NUMBER_OF_SHARDS));
