...
public class BlobTableAnalyzerTest extends BaseAnalyzerTest {
    ...
    public void testCreateBlobTableAutoExpand() {
        ...
        assertThat(analysis.tableSettings().settings().getAsInt(IndexMetaData.SETTING_NUMBER_OF_SHARDS, 0), is(10));
        assertThat(analysis.tableSettings().settings().get(IndexMetaData.SETTING_AUTO_EXPAND_REPLICAS), is("0-all"));
    }
    ...
    public void testCreateBlobTableWithPath() {
        ...
        assertThat(analysis.tableIdent().name(), is("screenshots"));
        assertThat(analysis.tableSettings().settings().get(BlobIndices.SETTING_INDEX_BLOBS_PATH), is("/tmp/crate_blob_data"));
    }
    ...
    public void testAlterBlobTableWithReplicas() throws Exception {
        ...
        assertThat(analysis.tableSettings().settings().getAsInt(IndexMetaData.SETTING_NUMBER_OF_REPLICAS, 0), is(2));
    }
    ...
    public void testCreateBlobTableWithParams() throws Exception {
        ...
        assertThat(analysis.tableIdent().name(), is("screenshots"));
        assertThat(analysis.tableIdent().schema(), is(BlobSchemaInfo.NAME));
        assertThat(analysis.tableSettings().settings().getAsInt(IndexMetaData.SETTING_NUMBER_OF_SHARDS, 0), is(2));
        assertThat(analysis.tableSettings().settings().get(IndexMetaData.SETTING_AUTO_EXPAND_REPLICAS), is("0-all"));
    }
