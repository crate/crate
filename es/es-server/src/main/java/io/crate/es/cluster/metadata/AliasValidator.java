...
import io.crate.es.indices.InvalidAliasNameException;
...
public class AliasValidator extends AbstractComponent {
    ...
    /**
     * Allows to validate an {@link io.crate.es.action.admin.indices.alias.Alias} and make sure
    ...
    /**
     * Allows to validate an {@link io.crate.es.cluster.metadata.AliasMetaData} and make sure
     * it's valid before it gets added to the index metadata. Doesn't validate the alias filter.
     * @throws IllegalArgumentException if the alias is not valid
     */
