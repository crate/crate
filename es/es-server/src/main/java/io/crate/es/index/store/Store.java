...
import io.crate.es.common.util.concurrent.RefCounted;
...
import io.crate.es.index.engine.CombinedDeletionPolicy;
...
public class Store extends AbstractIndexShardComponent implements Closeable, RefCounted {
    ...
    public static final class MetadataSnapshot implements Iterable<StoreFileMetaData>, Writeable {
        ...
        static LoadedMetadata loadMetadata(IndexCommit commit, Directory directory, Logger logger) throws IOException {
            ...
            try {
                ...
                if (maxVersion == null) {
                    maxVersion = io.crate.es.Version.CURRENT.minimumIndexCompatibilityVersion().luceneVersion;
                }
     ...
     *
     * @see MetadataSnapshot#recoveryDiff(io.crate.es.index.store.Store.MetadataSnapshot)
     */
    ...
    public interface OnClose extends Consumer<ShardLock> {
        OnClose EMPTY = new OnClose() {
            /**
             * This method is called while the provided {@link io.crate.es.env.ShardLock} is held.
    ...
    public void trimUnsafeCommits(final long lastSyncedGlobalCheckpoint, final long minRetainedTranslogGen,
                                  final io.crate.es.Version indexVersionCreated) throws IOException {
        metadataLock.writeLock().lock();
        try {
            ...
            if (indexVersionCreated.before(io.crate.es.Version.ES_V_6_5_1)) {
