...
import io.crate.es.cluster.routing.RecoverySource;
import io.crate.es.cluster.routing.RecoverySource.SnapshotRecoverySource;
import io.crate.es.cluster.routing.ShardRouting;
import io.crate.es.common.Booleans;
import io.crate.es.common.CheckedRunnable;
...
import io.crate.es.common.metrics.MeanMetric;
...
import io.crate.es.common.util.concurrent.AsyncIOProcessor;
...
import io.crate.es.index.cache.bitset.ShardBitsetFilterCache;
import io.crate.es.index.codec.CodecService;
import io.crate.es.index.engine.CommitStats;
import io.crate.es.index.engine.Engine;
import io.crate.es.index.engine.EngineConfig;
import io.crate.es.index.engine.EngineException;
import io.crate.es.index.engine.EngineFactory;
import io.crate.es.index.engine.RefreshFailedEngineException;
import io.crate.es.index.engine.Segment;
import io.crate.es.index.flush.FlushStats;
import io.crate.es.index.mapper.DocumentMapper;
import io.crate.es.index.mapper.DocumentMapperForType;
...
import io.crate.es.index.mapper.ParsedDocument;
import io.crate.es.index.mapper.RootObjectMapper;
import io.crate.es.index.mapper.SourceToParse;
import io.crate.es.index.mapper.Uid;
import io.crate.es.index.merge.MergeStats;
import io.crate.es.index.recovery.RecoveryStats;
import io.crate.es.index.refresh.RefreshStats;
import io.crate.es.index.seqno.ReplicationTracker;
import io.crate.es.index.seqno.SeqNoStats;
import io.crate.es.index.seqno.SequenceNumbers;
import io.crate.es.index.shard.PrimaryReplicaSyncer.ResyncTask;
import io.crate.es.index.store.Store;
import io.crate.es.index.store.Store.MetadataSnapshot;
import io.crate.es.index.store.StoreFileMetaData;
import io.crate.es.index.store.StoreStats;
import io.crate.es.index.translog.Translog;
import io.crate.es.index.translog.TranslogConfig;
import io.crate.es.indices.IndexingMemoryController;
import io.crate.es.indices.IndicesService;
import io.crate.es.indices.TypeMissingException;
import io.crate.es.indices.breaker.CircuitBreakerService;
import io.crate.es.indices.cluster.IndicesClusterStateService;
import io.crate.es.indices.recovery.PeerRecoveryTargetService;
import io.crate.es.indices.recovery.RecoveryFailedException;
...
import static io.crate.es.index.mapper.SourceToParse.source;
import static io.crate.es.index.seqno.SequenceNumbers.UNASSIGNED_SEQ_NO;
...
public class IndexShard extends AbstractIndexShardComponent implements IndicesClusterStateService.Shard {
     ...
     * See {@link io.crate.es.cluster.metadata.IndexMetaData#primaryTerm(int)}
     */
     ...
     * {@link io.crate.es.index.translog.TranslogDeletionPolicy} for details
     */
     ...
     * @see io.crate.es.indices.recovery.RecoveryTarget#indexTranslogOperations(List, int, long, long)
     */
