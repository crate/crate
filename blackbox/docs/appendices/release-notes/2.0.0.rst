...
----------------
 ...
 - The :ref:`conf-node-type` setting is set to ``true`` by default. This
 ...
 - All custom ``node.*`` style attributes must now be written as
   ...
   internally. Consult the :ref:`node attribute <conf-node-attrs>` docs for
   information.
