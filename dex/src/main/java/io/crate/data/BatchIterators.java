...
public class BatchIterators {
    ...
    public static <T, S> CompletableFuture<S> fold(BatchIterator<? extends T> it, BiFunction<T, S, S> step, S initialState) {
        S state = initialState;
        boolean allLoaded;
        try {
            while (it.moveNext()) {
                state = step.apply(it.currentElement(), state);
            }
    ...
    public static <T, S, R> CompletableFuture<R> collect(BatchIterator<? extends T> source,
                                                         Collector<T, S, R> collector) {
        BiConsumer<S, T> accumulator = collector.accumulator();
        return fold(source, (t, s) -> { accumulator.accept(s, t); return s; }, collector.supplier().get())
