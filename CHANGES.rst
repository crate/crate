...
==========
 ...
 - Serve admin UI from ``/admin/`` instead of ``/_plugins/crate-admin/``
...
================
 ...
 - Fixed an issue that causes ``UnsupportedFeatureException`` for operations
   on a table where array access is used inside generated columns. e.g.::
 ...
 - Fixed: throw correct error if one tries to create a table starting with ``_``.
 ...
 - Fixed an issue that causes ``UnhandledServerException`` on joins
   ...
   of both tables which are separated with the ``OR`` operator. e.g.::

     SELECT * FROM t1 join t2 ON t1.id = t2.id
     WHERE 'foo' = ANY(a.strArray) OR 'bar' = ANY(b.strArray)

 - Fixed: scalar functions ``longitude/latitude`` show misleading results.
 ...
 - Fixed an issue that causes ClassCastException on outer joins when
   ...
   the relation that may produce nulls. e.g.::
 ...
 - Fixed an issue that causes an error to be thrown if nested method
   calls are used in ``HAVING`` clause. e.g.::
