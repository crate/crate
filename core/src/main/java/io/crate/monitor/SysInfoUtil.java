...
public class SysInfoUtil {
    ...
    private void parseXenVendor(File releaseFile) {
        ...
        sysinfo.description = String.format("XenServer %s", sysinfo.vendorVersion);
    }
    ...
    private void parseRedHatVendor(File releaseFile) {
        parseGenericVendor(releaseFile);
        consumeFile(releaseFile, line -> {
            int start = line.indexOf("(") + 1;
            int len = line.lastIndexOf(")") + 1 - start;
            if (len > 0) {
                sysinfo.vendorCodeName = line.substring(start, len);
            }
    ...
    private void parseGenericVendor(File releaseFile) {
        ...
        consumeFile(releaseFile, line -> {
            ...
            for (int i = 0; i < sequence.length; i++) {
                ...
                if (len > 0) {
                    sysinfo.vendorVersion = line.substring(start, len);
                    return;
                }
            }
        });
    ...
    private void gatherWindows(String osName) {
        ...
        sysinfo.description = String.format("%s %s", sysinfo.vendor, sysinfo.vendorName);
    }
    ...
    private void gatherMacOs(String osName, String osVersion) {
        ...
        sysinfo.description = String.format("%s (%s)", sysinfo.vendorName, sysinfo.vendorCodeName);
    }
    ...
    private void gatherLinuxInfo(String osName) {
        ...
        if (sysinfo.description == null) {
            sysinfo.description = String.format("%s %s", sysinfo.vendor, sysinfo.vendorVersion);
        }
    }
    ...
    private static String getCanonicalArchitecture(String osArch) {
        String arch = osArch.toLowerCase().trim();
