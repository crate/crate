...
public class MultiFutureCallback<T> implements FutureCallback<T> {
    ...
    private final FutureCallback<List<T>> finalCallback;
    private final AtomicReference<Throwable> lastFailure = new AtomicReference<>();

    public MultiFutureCallback(int numCalls, FutureCallback<List<T>> finalCallback) {
    ...
    private void countdown() {
        if (counter.decrementAndGet() == 0) {
            ...
            if (throwable == null) {
                ...
                finalCallback.onFailure(throwable);
            }
        }
    }
}
