...
public class SelectOrderByIntegrationTest extends IntegTestCase {
    ...
    @Test
    public void test_order_by_ambiguity_between_sentinel_and_null_no_doc_values() {
        execute("create table t1 (i real storage with (columnstore=false), d double storage with (columnstore=false))");
        execute("insert into t1 (i, d) values " +
            "(cast('Infinity' as real), 1.0), " +
            "(1.0, cast('Infinity' as double)), " +
            "(NULL, cast('-Infinity' as double)), " +
