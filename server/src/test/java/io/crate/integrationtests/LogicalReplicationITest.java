...
public class LogicalReplicationITest extends ESTestCase {
    ...
    public void test_create_publication_for_concrete_table() {
        ...
        var response = executeOnPublisher(
            "SELECT oid, pubname, pubowner, puballtables, pubinsert, pubupdate, pubdelete" +
            " FROM pg_publication ORDER BY pubname");
        assertThat(printedTable(response.rows()), is("-119974068| pub1| -450373579| false| true| true| true\n"));
    }

    @Test
    public void test_create_publication_for_all_tables() {
        executeOnPublisher("CREATE TABLE doc.t1 (id INT)");
        executeOnPublisher("CREATE TABLE doc.t2 (id INT)");
        executeOnPublisher("CREATE PUBLICATION pub1 FOR ALL TABLES");
        var response = executeOnPublisher(
            "SELECT oid, pubname, pubowner, puballtables, pubinsert, pubupdate, pubdelete" +
            " FROM pg_publication ORDER BY pubname");
        assertThat(printedTable(response.rows()), is("284890074| pub1| -450373579| true| true| true| true\n"));
        // TODO: check published tables
    }
