...
public class LuceneQueryBuilderIntegrationTest extends IntegTestCase {
    ...
    public void debug() throws Exception {
        // More columns to make
        // GenericFunctionQuery -> context ->SegmentReader -> FieldInfo
        // heavier and maybe hit OOM earlier.
        // UPD: No, it didn't help
        execute("create table t (a int, b int, c int, d int, e int, f int, g int, h int, " +
            " o object(ignored))");
        ...
        for (int i = 1; i < 10_000_000; i++) {
            ...
            for (int j = 0; j < 5; j++) {
                ...
                execute("select * from t where o['dyn'] = ANY([0, " + i + "]) " +
                    " and a is null " +
                    " and b is null " +
                    " and c is null" +
                    " and d is null");
