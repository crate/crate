...
public class JoinTest extends CrateDummyClusterServiceUnitTest {
    ...
    public void test_relationnames_order_in_nested_loop_join() throws Exception {
        ...
        assertThat(result.baseTables().get(0).relationName().toString()).isEqualTo("doc.t2");
        assertThat(result.baseTables().get(1).relationName().toString()).isEqualTo("doc.t1");
        assertThat(result.baseTables().get(2).relationName().toString()).isEqualTo("doc.t3");
    }
}
