...
public class RelationNamesInLogicalPlanTest extends CrateDummyClusterServiceUnitTest {
    ...
    public void setup() throws Exception {
        ...
        var t1Collect = new Collect(t1Relation, List.of(x), WhereClause.MATCH_ALL, 1L, DataTypes.INTEGER.fixedSize(), idAllocator.nextId());
        Symbol t1Output = t1Alias.getField(x.column(), Operation.READ, true);
        t1Rename = new Rename(List.of(t1Output), t1Alias.relationName(), t1Alias, t1Collect, idAllocator.nextId());
        ...
        var t2Collect = new Collect(t2Relation, List.of(x), WhereClause.MATCH_ALL, 1L, DataTypes.INTEGER.fixedSize(), idAllocator.nextId());
        Symbol t2Output = t2Alias.getField(y.column(), Operation.READ, true);
        t2Rename = new Rename(List.of(t2Output), t2Alias.relationName(), t2Alias, t2Collect, idAllocator.nextId());
    }

    @Test
    public void test_relationnames_are_based_on_sources_in_hashjoin() throws Exception {
        var hashJoin = new HashJoin(t1Rename, t2Rename, e.asSymbol("x = y"), idAllocator.nextId());
    ...
    public void test_relationnames_are_based_on_sources_in_union() {
        var union = new Union(t1Rename, t2Rename, List.of(), idAllocator.nextId());
    ...
    public void test_relationnames_are_based_on_sources_in_table_function() {
        ...
        var tableFunction = new TableFunction(tableFunctionRelation, List.of(), new WhereClause(null), idAllocator.nextId());
    ...
    public void test_relationnames_are_based_on_sources_in_collect() {
        var collect = new Collect(t1Relation, List.of(), new WhereClause(null), 1,1, idAllocator.nextId());
    ...
    public void test_relationnames_are_based_on_sources_in_get() {
        var get = new Get(t1Relation, new DocKeys(List.of(List.of()), false, false, 1, null), null, List.of(), 1, idAllocator.nextId());
    ...
    public void test_relationnames_are_based_on_sources_in_count() {
        ...
        var count = new Count(function, t1Relation, new WhereClause(null), idAllocator.nextId());
