...
public class RewriteMixedDistinctAggToGroupByTest extends CrateDummyClusterServiceUnitTest {
    ...
    private void setTableStats(long numDocs) {
        e.updateTableStats(Map.of(
            e.resolveTableInfo("t1").ident(),
            new Stats(numDocs, 0L, Map.of())
        ));
    }
    ...
    @Test
    public void test_multiple_distinct_aggregates_on_same_column_are_mixed_with_non_distinct() {
        // SELECT sum(i), count(distinct x), avg(distinct x) FROM t1
        var collectXI = e.logicalPlan("SELECT x, i FROM t1");
        var sumI = (Function) e.asSymbol("sum(i)");
        var countX = (Function) e.asSymbol("count(distinct x)");
        var avgX = (Function) e.asSymbol("avg(distinct x)");
        var hashAgg = new HashAggregate(collectXI, List.of(sumI, countX, avgX));

        assertApplied(
            hashAgg,
            """
            HashAggregate[sum(i), count(DISTINCT x), avg(DISTINCT x)]
              └ GroupHashAggregate[x | sum(i)]
    ...
    @Test
    public void test_distinct_column_same_as_non_distinct_aggregate_column() {
        // SELECT sum(x), count(distinct x) FROM t1
        var collectX = e.logicalPlan("SELECT x FROM t1");
        var sumX = (Function) e.asSymbol("sum(x)");
        var countX = (Function) e.asSymbol("count(distinct x)");
        var hashAgg = new HashAggregate(collectX, List.of(sumX, countX));

        assertApplied(
            hashAgg,
            """
            HashAggregate[sum(x), count(DISTINCT x)]
              └ GroupHashAggregate[x | sum(x)]
    ...
    @Test
    public void test_cannot_apply_when_all_aggregates_are_distinct_over_same_column() {
        // SELECT count(distinct x), avg(distinct x) FROM t1 -- handled by RewriteDistinctAggToGroupBy instead
        var collectX = e.logicalPlan("SELECT x FROM t1");
        var countX = (Function) e.asSymbol("count(distinct x)");
        var avgX = (Function) e.asSymbol("avg(distinct x)");
        var hashAgg = new HashAggregate(collectX, List.of(countX, avgX));
    ...
    @Test
    public void test_cannot_apply_to_high_cardinality_column() {
        // SELECT sum(x), count(distinct a) FROM t1
        setTableStats((long) (RewriteMixedDistinctAggToGroupBy.MAX_TABLE_SIZE + 1));
