...
public class CloseIndexIT extends IntegTestCase {
    ...
    private static void assertIndexIsClosed(final RelationName relation) {
        ...
        for (IndexMetadata indexMetadata : indices) {
            ...
            assertThat(clusterState.blocks().hasIndexBlock(index.uuid(), IndexMetadata.INDEX_CLOSED_BLOCK)).isTrue();
            assertThat(clusterState.blocks().indices().getOrDefault(index.uuid(), emptySet()).stream()
