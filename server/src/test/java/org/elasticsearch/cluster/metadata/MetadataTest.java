/*
 * Licensed to Crate.io GmbH ("Crate") under one or more contributor
 * license agreements.  See the NOTICE file distributed with this work for
 * additional information regarding copyright ownership.  Crate licenses
 * this file to you under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.  You may
 * obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.  See the
 * License for the specific language governing permissions and limitations
 * under the License.
 *
 * However, if you have executed another commercial license agreement
 * with Crate these terms will supersede the license and you may use the
 * software solely pursuant to the terms of the relevant commercial agreement.
 */

package org.elasticsearch.cluster.metadata;

import static org.assertj.core.api.Assertions.assertThat;

import java.util.List;
import java.util.Map;

import org.elasticsearch.cluster.Diff;
import org.elasticsearch.cluster.metadata.IndexMetadata.State;
import org.elasticsearch.cluster.metadata.Metadata.Builder;
import org.elasticsearch.common.io.stream.BytesStreamOutput;
import org.elasticsearch.common.io.stream.NamedWriteableAwareStreamInput;
import org.elasticsearch.common.io.stream.StreamInput;
import org.elasticsearch.common.settings.Settings;
import org.junit.Test;

import io.crate.metadata.Reference;
import io.crate.metadata.RelationName;
import io.crate.sql.tree.ColumnPolicy;
import io.crate.test.integration.CrateDummyClusterServiceUnitTest;

public class MetadataTest extends CrateDummyClusterServiceUnitTest {

    @Test
    public void test_add_relation_and_schemas_streaming() throws Exception {
        Builder builder = Metadata.builder();
        RelationName relationName = new RelationName("s", "tbl");
        List<Reference> columns = List.of();
        builder.addTable(
            relationName,
            columns,
            Settings.EMPTY,
            null,
            ColumnPolicy.STRICT,
            null,
            Map.of(),
            List.of(),
            List.of(),
            State.OPEN,
            List.of("indexuuid1")
        );
        Metadata metadata = builder.build();
        assertThat(metadata.schemas()).hasSize(1);
        SchemaMetadata schemaMetadata = metadata.schemas().get("s");
        assertThat(schemaMetadata).isNotNull();
        assertThat(schemaMetadata.relations()).hasSize(1);

        {
            BytesStreamOutput out = new BytesStreamOutput();
            metadata.writeTo(out);

            StreamInput in = out.bytes().streamInput();
            in = new NamedWriteableAwareStreamInput(in, writableRegistry());
            Metadata metadataFrom = Metadata.readFrom(in);
            assertThat(metadataFrom.schemas()).isEqualTo(metadata.schemas());
        }
        {
            Metadata emptyMetadata = Metadata.builder().build();
            Diff<Metadata> diff = metadata.diff(emptyMetadata);

            BytesStreamOutput out = new BytesStreamOutput();
            diff.writeTo(out);
            try (var in = new NamedWriteableAwareStreamInput(out.bytes().streamInput(), writableRegistry())) {
                Diff<Metadata> diffFrom = Metadata.readDiffFrom(in);

                Metadata diffed = diffFrom.apply(emptyMetadata);
                assertThat(diffed.schemas()).isEqualTo(metadata.schemas());
            }
        }
    }
}

