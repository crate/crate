...
import org.elasticsearch.cluster.health.HealthStatus;
...
public class ClusterHealthIT extends IntegTestCase {
    ...
    public void testSimpleLocalHealth() throws Exception {
        ...
        for (final String node : cluster().getNodeNames()) {
            ...
            final ClusterHealthResponse health = FutureUtils.get(client(node).health(
            ...
            assertThat(health.getStatus()).as("health status on " + node).isEqualTo(HealthStatus.GREEN);
        }
    }
    ...
    public void testHealth() {
        ...
        ClusterHealthResponse healthResponse = FutureUtils.get(client().health(
            new ClusterHealthRequest()
                .waitForGreenStatus()
                .timeout("10s")
            ));
        assertThat(healthResponse.isTimedOut()).isFalse();
        assertThat(healthResponse.getStatus()).isEqualTo(HealthStatus.GREEN);
        ...
        healthResponse = FutureUtils.get(client().health(
            new ClusterHealthRequest()
                .waitForGreenStatus()
                .timeout("10s")
            ));
        assertThat(healthResponse.isTimedOut()).isFalse();
        assertThat(healthResponse.getStatus()).isEqualTo(HealthStatus.GREEN);
        assertThat(healthResponse.getIndices().get("test1").getStatus()).isEqualTo(HealthStatus.GREEN);
        ...
        assertThat(healthResponse.getStatus()).isEqualTo(HealthStatus.YELLOW);
        assertThat(healthResponse.getIndices().get("test1").getStatus()).isEqualTo(HealthStatus.GREEN);
        assertThat(healthResponse.getIndices().get("test2").getStatus()).isEqualTo(HealthStatus.YELLOW);
    }
    ...
    public void testHealthWithClosedIndices() throws Exception {
        ...
        {
            ClusterHealthResponse response = FutureUtils.get(client().health(
                new ClusterHealthRequest()
                    .waitForNoRelocatingShards(true)
                    .waitForNoInitializingShards(true)
                    .waitForYellowStatus()
                ), REQUEST_TIMEOUT);
            assertThat(response.getStatus()).isEqualTo(HealthStatus.YELLOW);
            assertThat(response.isTimedOut()).isFalse();
            assertThat(response.getIndices()).hasSize(3);
            assertThat(response.getIndices().get(table_1).getStatus()).isEqualTo(HealthStatus.GREEN);
            assertThat(response.getIndices().get(table_2).getStatus()).isEqualTo(HealthStatus.GREEN);
            assertThat(response.getIndices().get(table_3).getStatus()).isEqualTo(HealthStatus.YELLOW);
        }
...

        {
            ClusterHealthResponse response = FutureUtils.get(
                client().health(new ClusterHealthRequest().waitForGreenStatus()),
                REQUEST_TIMEOUT
            );
            assertThat(response.getStatus()).isEqualTo(HealthStatus.GREEN);
            assertThat(response.isTimedOut()).isFalse();
            assertThat(response.getIndices()).hasSize(3);
            assertThat(response.getIndices().get(table_1).getStatus()).isEqualTo(HealthStatus.GREEN);
            assertThat(response.getIndices().get(table_2).getStatus()).isEqualTo(HealthStatus.GREEN);
            assertThat(response.getIndices().get(table_3).getStatus()).isEqualTo(HealthStatus.GREEN);
        }
    }
            ...
            public void run() {
                while (finished.get() == false) {
                    ClusterHealthResponse health = FutureUtils.get(
                        client().health(new ClusterHealthRequest()));
                    assertThat(health.getStatus()).isNotEqualTo(HealthStatus.RED);
                }
            }
        };
    ...
    public void testWaitForEventsRetriesIfOtherConditionsNotMet() throws Exception {
        ...
        final CompletableFuture<ClusterHealthResponse> healthResponseFuture = client().health(
            new ClusterHealthRequest()
                .waitForEvents(Priority.LANGUID)
                .waitForGreenStatus()
        );
        ...
        var clusterHealthResponse = FutureUtils.get(client().health(new ClusterHealthRequest().waitForGreenStatus()));
    ...
    public void testHealthOnMasterFailover() throws Exception {
        ...
        for (int i = 0; i < iterations; ++i) {
            responseFutures.add(client(node).health(new ClusterHealthRequest().waitForEvents(Priority.LANGUID)
        ...
        for (var responseFuture : responseFutures) {
            assertThat(responseFuture.get().getStatus()).isSameAs(HealthStatus.GREEN);
        }
    }
}
