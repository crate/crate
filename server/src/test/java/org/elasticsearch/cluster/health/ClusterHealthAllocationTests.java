...
public class ClusterHealthAllocationTests extends ESAllocationTestCase {
    ...
    public void testClusterHealth() {
        ...
        assertThat(getClusterHealthStatus(clusterState)).isEqualTo(HealthStatus.GREEN);
        ...
        assertThat(getClusterHealthStatus(clusterState)).isEqualTo(HealthStatus.RED);

        clusterState = addNode(clusterState, "node_d1", false);
        assertThat(clusterState.nodes().getDataNodes().size()).isEqualTo(1);
        clusterState = applyStartedShardsUntilNoChange(clusterState, allocation);
        assertThat(getClusterHealthStatus(clusterState)).isEqualTo(HealthStatus.YELLOW);
    ...
    private HealthStatus getClusterHealthStatus(ClusterState clusterState) {
