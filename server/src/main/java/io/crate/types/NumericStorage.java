...
import io.crate.expression.reference.doc.lucene.NumericDocValueExpression;
...
public final class NumericStorage extends StorageSupport<BigDecimal> {
    ...
    public ValueIndexer<? super BigDecimal> valueIndexer(RelationName table,
        ...
        assert ref.valueType() instanceof NumericType : "ValueIndexer on NumericStorage can only be used for numeric types";
        NumericType numericType = (NumericType) ref.valueType();
        Integer precision = numericType.numericPrecision();
        if (precision == null) {
            return new UnlimitedNumericIndexer(ref);
    ...
    private static class CompactNumericIndexer extends BaseNumericIndexer {
        ...
        public void indexValue(@NotNull BigDecimal value, IndexDocumentBuilder docBuilder) throws IOException {
            ...
            long longValue = unscaledValue.longValueExact();
            ...
            // TODO: can we store longValue here?
    ...
    private static class FixedNumericIndexer extends BaseNumericIndexer {
        ...
        private FixedNumericIndexer(Reference ref, int precision) {
            ...
            // this.fieldType.setDimensions(1, 16);
    ...
    private static class UnlimitedNumericIndexer extends BaseNumericIndexer {
        ...
        private UnlimitedNumericIndexer(Reference ref) {
            super(ref);
        }
        ...
        public void indexValue(@NotNull BigDecimal value, IndexDocumentBuilder docBuilder) throws IOException {
            ...
            if (ref.hasDocValues()) {
                ...
                Field field = new Field(DocSysColumns.FieldNames.NAME, name, DocSysColumns.FieldNames.FIELD_TYPE);
    ...
    public static LuceneCollectorExpression<BigDecimal> getCollectorExpression(String fqn, NumericType type) {
        ...
        return new NumericDocValueExpression<>(fqn) {
