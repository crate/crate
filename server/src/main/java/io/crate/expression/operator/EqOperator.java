        ...
        for (Map.Entry<String, Object> entry : value.entrySet()) {
            ...
            if (DataTypes.isArray(innerType)) {
                innerQuery = termsAndGenericFilter(
                    eq, childColumn.fqn(), innerType, (Collection<?>) entry.getValue(), context, childRef.hasDocValues(), childRef.indexType());
            } else {
                innerQuery = fromPrimitive(innerType, childColumn.fqn(), entry.getValue(), childRef.hasDocValues(), childRef.indexType());
            }
            if (innerQuery == null) {
                continue;
            }
