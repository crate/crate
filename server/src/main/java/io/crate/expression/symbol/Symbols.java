...
public final class Symbols {
    ...
    public static <V> V lookupValueByColumn(RelationName relationName, Map<? extends Symbol, V> valuesBySymbol, ColumnIdent column) {
        for (Map.Entry<? extends Symbol, V> entry : valuesBySymbol.entrySet()) {
            ...
            if (key instanceof Reference ref && ref.column().equals(column) && ref.ident().tableIdent().equals(relationName)) {
                return entry.getValue();
            }
            if (key instanceof ScopedSymbol scopedSymbol && scopedSymbol.column().equals(column) && scopedSymbol.relation().equals(relationName)) {
                return entry.getValue();
            }
        }
        return null;
