...
public final class RewriteDistinctAggToGroupBy implements Rule<HashAggregate> {
    ...
    private boolean canBeOptimized(HashAggregate agg) {
    ...
    private boolean hasFilter(Function aggregate) {
    ...
    public LogicalPlan apply(HashAggregate aggregate, Captures captures, Rule.Context context) {
        ...
        // Resolving the source is only reached once the pattern matched, so only for queries that
        // actually contain a distinct aggregate.
    ...
    private boolean groupByPresent(LogicalPlan plan, Symbol groupKey) {
