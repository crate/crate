...
public class LogicalPlanner {
    ...
    static class PlanBuilder extends AnalyzedRelationVisitor<List<Symbol>, LogicalPlan> {
        ...
        public LogicalPlan visitJoinRelation(JoinRelation joinRelation, List<Symbol> outputs) {
            var lhsOutputs = mergeOutputs(outputs, joinRelation.left());
            var rhsOutputs = mergeOutputs(outputs, joinRelation.right());
        ...
        static List<Symbol> mergeOutputs(List<Symbol> outputs, AnalyzedRelation relation) {
            ...
            LogicalPlan source = JoinPlanBuilder.buildJoinTree(
                ...
                rel -> {
                    ...
                    if (relation.from().size() == 1) {
                        ...
                        List<Symbol> mergedOutputs = mergeOutputs(toCollect, rel);
