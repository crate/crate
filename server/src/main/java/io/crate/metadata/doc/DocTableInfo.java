...
public class DocTableInfo implements TableInfo, ShardedTable, StoredTable {
    ...
    public DocTableInfo(RelationName ident,
        ...
        assert versionCreated.after(Version.V_EMPTY) : "Table " + ident + " must have a versionCreated";
    ...
    private List<String> getIndexNames(Metadata metadata, Predicate<IndexMetadata> predicate) {
        RelationMetadata relation = metadata.getRelation(ident);
        if (relation instanceof RelationMetadata.Table table) {
            List<String> uuids = table.indexUUIDs();
            ArrayList<String> indexNames = new ArrayList<>(uuids.size());
            for (int i = 0; i < table.indexUUIDs().size(); i++) {
                String indexUUID = table.indexUUIDs().get(i);
                IndexMetadata indexMetadata = metadata.indexByUUID(indexUUID);
                if (indexMetadata != null && predicate.test(indexMetadata)) {
    ...
    public String[] concreteIndices(Metadata metadata) {
        return getIndexNames(metadata, x -> true).toArray(String[]::new);
    }

    public String[] concreteOpenIndices(Metadata metadata) {
        return getIndexNames(metadata, x -> x.getState() == State.OPEN).toArray(String[]::new);
    }
    ...
    public Metadata.Builder writeTo(Metadata metadata,
        ...
        // - oid for new columns?
        ArrayList<String> indexUUIDs = new ArrayList<String>();
