...
public class SysClusterHealth {
    ...
    public static Iterable<ClusterHealth> compute(ClusterState clusterState, long numPendingTasks) {
        ...
        final HealthStatus clusterHealth = !blocksYellow.isEmpty() ? HealthStatus.YELLOW : HealthStatus.GREEN;
        ...
        HealthStatus health = clusterHealth;
        ...
        for (var tableHealth : TableHealth.compute(clusterState)) {
            if (tableHealth.health().severity() > health.severity()) {
                ...
                if (health == HealthStatus.RED || tableHealth.missingShards() > 0) {
                    finalDescription = "One or more tables are missing shards";
                } else if (health == HealthStatus.YELLOW) {
    ...
    public record ClusterHealth(HealthStatus health,
                                String description,
                                long missingShards,
