...
public class TransportDropServer extends TransportMasterNodeAction<DropServerRequest, AcknowledgedResponse> {
    ...
    @Override
    protected void masterOperation(DropServerRequest request,
                                   ClusterState state,
                                   ActionListener<AcknowledgedResponse> listener) throws Exception {
        if (state.nodes().getMinNodeVersion().before(Version.V_5_7_0)) {
            throw new IllegalStateException(
                "Cannot execute DROP SERVER while there are <5.7.0 nodes in the cluster");
        }

        var task = new DropServerTask(request);
        task.completionFuture().whenComplete(listener);
        clusterService.submitStateUpdateTask("drop_server", task);
    }

    @Override
    protected ClusterBlockException checkBlock(DropServerRequest request, ClusterState state) {
