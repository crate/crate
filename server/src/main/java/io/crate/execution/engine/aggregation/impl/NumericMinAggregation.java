...
/**
 * NumericMinAggregation implements the `min()` aggregation function for the {@link NumericType},
 * by taking advantage of the column store.
 */
public class NumericMinAggregation extends NumericMinMaxBase {
    ...
    protected void setNewStateValue(BigDecimalValueWrapper state, BigDecimal value, RamAccounting ramAccounting) {
        ...
        if (state.value() == null) {
            ...
            newValue = state.value().min(value);
