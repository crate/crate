...
public abstract class MaximumAggregation extends AggregationFunction<Object, Object> {
    ...
    private static class VariableMaximumAggregation extends MaximumAggregation {
        ...
        public DocValueAggregator<?> getDocValueAggregator(LuceneReferenceResolver referenceResolver,
            ...
            DataType<?> valueType = reference.valueType();
            Integer numericPrecision = valueType.numericPrecision();
            if (valueType instanceof NumericType && numericPrecision != null) {
                return getNumericDocValueAggregator(
                    aggregationReferences,
                    (ramAccounting, state, bigDecimal) -> {
                        return reduce(ramAccounting, state, bigDecimal);
                    }
                );
            }
            return null;
