...
public class IndexWriterProjector implements Projector {
    ...
    public IndexWriterProjector(ClusterService clusterService,
        ...
        final Map<Set<String>, Reference[]> targetColumnCache = new HashMap<>();
        final AtomicReference<Reference[]> currentTargetColumns = new AtomicReference<>();
        Function<ShardId, ShardUpsertRequest> createRequest = shardId -> {
            Reference[] insertColumns = currentTargetColumns.get();
        ...
        ItemFactory<ShardUpsertRequest.Item> itemFactory = (id, pkValues, autoGeneratedTimestamp) -> {
            ...
            Reference[] targetColumns = targetColumnCache.computeIfAbsent(doc.keySet(), docKeys -> {
                Reference[] references = new Reference[docKeys.size()];
                int i = 0;
            ...
            Reference[] prevReferences = currentTargetColumns.get();
            if (prevReferences != null && !Arrays.deepEquals(prevReferences, targetColumns)) {
                currentTargetColumns.set(null);
                throw new BatchIterators.NewPartitionException();
            }
            currentTargetColumns.set(targetColumns);
            try {
                Object[] values = new Object[targetColumns.length];
                for (int j = 0; j < targetColumns.length; j++) {
                    Reference reference = targetColumns[j];
