...
public final class GroupRowsByShard<TReq extends ShardRequest<TReq, TItem>, TItem extends ShardRequest.Item>
    ...
    public TItem apply(ShardedRequests<TReq, TItem> shardedRequests, Row row, Boolean propagateError) {
        ...
        try {
            ...
            IndexMetadata indexMetadata = state.metadata().getIndex(
            ...
            ShardLocation shardLocation = indexMetadata == null
                ? null
                : getShardLocation(state, indexMetadata, id, routing);
            if (shardLocation == null) {
                ...
                if (item instanceof IndexItem indexItem) {
                    constraintsChecker.accept(partitionName, indexItem);
                }
    ...
    @Nullable
    private ShardLocation getShardLocation(ClusterState state,
                                           IndexMetadata indexMetadata,
                                           String id,
                                           @Nullable String routing) {
        try {
            int shardId = OperationRouting.generateShardId(
                indexMetadata,
                id,
                routing
            );
    ...
    void reResolveShardLocations(ShardedRequests<TReq, TItem> requests, int tableOID) {
        ...
        while (entryIt.hasNext()) {
            ...
            List<ShardedRequests.ItemAndRoutingAndSourceInfo<TItem>> items = entry.getValue();
            ...
            while (it.hasNext()) {
                ...
                IndexMetadata indexMetadata = metadata.getIndex(
                    // on a mixed cluster of 6.2 and 6.3 tableOIDs could still be unassigned, must fall back to relationName based lookup
