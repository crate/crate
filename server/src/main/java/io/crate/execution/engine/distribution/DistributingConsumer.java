...
public class DistributingConsumer implements RowConsumer {
    ...
    private final TransportKillJobsNodeAction transportKillJobs;
    ...
    public DistributingConsumer(Executor responseExecutor,
                                ...
                                int pageSize,
                                TransportKillJobsNodeAction transportKillJobs) {
        ...
        this.transportKillJobs = transportKillJobs;
        ...
        public void accept(DistributedResultResponse resp, Throwable err) {
            ...
            if (err instanceof ConnectTransportException) {
                if (retry < 3) {
                    ...
                    String reason = "Couldn't send result to downstream node: " + err.getMessage();
                    LOGGER.trace("Sending kill because of reason={}", reason);
                    KillJobsNodeRequest killReq = new KillJobsNodeRequest(
                        List.of(),
                        List.of(jobId),
                        Role.CRATE_USER.name(),
                        reason
                    );
                    transportKillJobs.execute(killReq).whenComplete((_, killErr) -> {
                        if (killErr != null) {
                            LOGGER.trace("Error during kill", killErr);
                        }
                    });
                }
            }
