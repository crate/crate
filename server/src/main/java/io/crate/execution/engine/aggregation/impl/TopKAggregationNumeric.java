...
public class TopKAggregationNumeric extends AggregationFunction<TopKAggregationNumeric.TopKState, List<Object>> {
    ...
    static {
        DataTypes.register(TopKAggregationNumeric.TopKStateType.ID, in -> TopKAggregationNumeric.TopKStateType.INSTANCE);
    }

    public static void register(Functions.Builder builder) {
        builder.add(
            DEFAULT_SIGNATURE,
            TopKAggregationNumeric::new
        );
    ...
    private TopKAggregationNumeric(Signature signature, BoundSignature boundSignature) {
    ...
    public TopKState newState(RamAccounting ramAccounting,
        ...
        return new TopKState(new LongsSketch(32), 8);
    }
    ...
    public TopKState iterate(RamAccounting ramAccounting,
        ...
        if (state.itemsSketch.isEmpty() && args.length == 2) {
            ...
            state = new TopKState(new LongsSketch(maxMapSize(limit)), limit);
        }
        Long value = (Long) args[0].value();
    ...
    public List<Object> terminatePartial(RamAccounting ramAccounting, TopKState state) {
        LongsSketch.Row[] frequentItems = state.itemsSketch.getFrequentItems(ErrorType.NO_FALSE_NEGATIVES);
    ...
    static final class TopKStateType extends DataType<TopKState> implements Streamer<TopKState> {
        ...
        public TopKState readValueFrom(StreamInput in) throws IOException {
            ...
            return new TopKState(LongsSketch.getInstance(Memory.wrap(in.readByteArray())), limit);
        }

        @Override
        public void writeValueTo(StreamOutput out, TopKState state) throws IOException {
            out.writeInt(state.limit);
            out.writeByteArray(state.itemsSketch.toByteArray());
        }
