...
final class GroupByOptimizedIterator {
     ...
     * <p>
     * The returned map holds the result: it is keyed by the resolved key values - one list element
     * per key column, in the order of the keys, e.g. {@code ["foo", "Austria"]} - and its value is
     * one aggregation state per aggregator, e.g. {@code [count_state, avg_state]} and it spans all
     * segments.
     * <p>
     * Additionally, a helper {@link Map} structure (which is not returned) is maintained within a segment,
     * using the {@link #getNested(LongObjectHashMap, long[])} method. The same groups are stored in this map,
     * one level per key column, where each level is keyed by the ordinal of its key column in that segment:
     * {@code ord(key0) -> ord(key1) -> ... -> states}. The per-segment key is the whole tuple of ordinals,
     * spread over the nested levels, therefore, routing a doc to its group uses longs and performs one lookup
     * per key column until you "descend" to the `states`.
     * <p>
