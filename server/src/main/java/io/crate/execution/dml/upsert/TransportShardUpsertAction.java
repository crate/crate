    ...
    private IndexItemResult indexItemForUpdate(Indexer indexer,
        ...
        for (int retryCount = 0; retryCount < MAX_RETRY_LIMIT; retryCount++) {
            try {
                boolean isRetry = retryCount > 0 || request.isRetry();
                AtomicLong version = new AtomicLong();
                var indexItem = lookupAndConvertToInsert(
                    indexer, item, indexShard, tableInfo, partitionValues, seqNo, primaryTerm, isRetry, version);
                final long startTime = System.nanoTime();
                List<Reference> newColumns = indexer.collectSchemaUpdates(indexItem);
                ...
                ParsedDocument parsedDoc = indexer.index(indexItem);
                Term uid = new Term(SysColumns.Names.ID, Uid.encodeId(indexItem.id()));
                ...
                Engine.Index index = new Engine.Index(
                    ...
                    isRetry,
                    indexItem.seqNo(),
                    indexItem.primaryTerm()
                );
                IndexResult result = indexShard.index(index);
                switch (result.getResultType()) {
                    case SUCCESS:
                        Object[] replicaInsertValues = indexer.addGeneratedValues(indexItem);
