...
public class TransportShardUpsertAction extends TransportShardAction<ShardUpsertRequest, ShardUpsertRequest.Item> {
        ...
        for (int retryCount = 0; retryCount < MAX_RETRY_LIMIT; retryCount++) {
            try {
                ...
                if (tryInsertFirst) {
                    ...
                    // Do we need to fetch the stored row in order to perform an update
                    boolean requiresSource = item.updateAssignments() != null && item.updateAssignments().length > 0;
                    Doc doc = getDocument(
                        indexShard,
                        item.id(),
                        item.version(),
                        item.seqNo(),
                        item.primaryTerm(),
                        requiresSource ? StoredRowLookup.create(actualTable) : null);
    ...
    private static Doc getDocument(IndexShard indexShard,
                                   ...
                                   long seqNo,
                                   long primaryTerm,
                                   @Nullable StoredRowLookup storedRowLookup) {
