...
public class TransportShardUpsertAction extends TransportShardAction<
    ...
    protected WritePrimaryResult<UpsertReplicaRequest, ShardResponse> processRequestItems(IndexShard indexShard,
        ...
        List<ScopedRef> insertColumns = new ArrayList<>();
        if (request.insertColumns() != null) {
            for (var ref : request.insertColumns()) {
                ScopedRef updatedRef = tableInfo.getReference(ref.column());
        ...
        for (ShardUpsertRequest.Item item : request.items()) {
            ...
            try {
                ...
                IndexItemResult indexItemResult = pureUpdate
                    ? update(indexer, request, item, indexShard, tableInfo, partitionValues)
                    : insertOrUpdate(indexer, request, item, indexShard, tableInfo, partitionValues, rawIndexer);
    ...
    protected WriteReplicaResult processRequestItemsOnReplica(IndexShard indexShard, UpsertReplicaRequest request) throws IOException {
        List<ScopedRef> columns = request.columns();
        ...
        List<ScopedRef> targetColumns = Lists.map(columns,
            ref -> {
                ScopedRef updatedRef = tableInfo.getReference(ref.column());
        ...
        for (UpsertReplicaRequest.Item item : request.items()) {
            ...
            List<ScopedRef> newColumns = rawIndexer != null ? rawIndexer.collectSchemaUpdates(item) : indexer.collectSchemaUpdates(item);
    ...
    private IndexItemResult update(Indexer indexer,
        ...
        for (int retryCount = 0; retryCount < MAX_RETRY_LIMIT; retryCount++) {
            try {
                ...
                List<ScopedRef> newColumns = indexer.collectSchemaUpdates(newIndexItem);
                ...
                Object[] replicaItems = indexer.addGeneratedValues(newIndexItem);
    ...
    protected IndexItemResult insert(RelationName tableName,
        ...
        final long startTime = System.nanoTime();
        List<ScopedRef> newColumns = rawIndexer == null
        ...
        Object[] replicaInsertValues = rawIndexer == null ?
            indexer.addGeneratedValues(item) : rawIndexer.addGeneratedValues(item);
        return indexParsedDoc(
    ...
    private DocTableInfo updateSchema(RelationName tableName,
                                      List<ScopedRef> newColumns) throws InterruptedException, ExecutionException {
