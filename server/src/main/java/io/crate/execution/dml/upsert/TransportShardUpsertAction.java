...
public class TransportShardUpsertAction extends TransportShardAction<
    ...
    protected WritePrimaryResult<UpsertReplicaRequest, ShardResponse> processRequestItems(IndexShard indexShard,
        ...
        List<ColumnIdent> insertColumnNamesForReplica = insertColumnsForReplica.stream().map(Reference::column).toList();
        List<Reference> insertOnConflictColumns = indexer.indexOrder().stream().map(ref -> {
            int idx = insertColumnNamesForReplica.indexOf(ref.column());
            if (idx != -1 && !insertColumnsForReplica.get(idx).valueType().equals(ref.valueType())) {
                return insertColumnsForReplica.get(idx);
            }
            return ref;
        }).toList();
        ...
        for (int retryCount = 0; retryCount < MAX_RETRY_LIMIT; retryCount++) {
            try {
                ...
                if (!fallBackToUpdate) {
                    ...
                    IndexItemResult indexItemResult = insert(
                        tableInfo.ident(),
                        indexer,
                        request,
                        item,
                    ...
                    if (indexer.onConflictIndexer() != null) {
                        ...
                        for (int i = 0; i < order.size(); i++) {
                            Reference insertColumn = order.get(i);
