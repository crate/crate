...
public class AnalyzedInsertStatement implements AnalyzedStatement {
    ...
    AnalyzedInsertStatement(AnalyzedRelation subQueryRelation,
        ...
        Map<ColumnIdent, GeneratedReference> generatedColumns =
            Maps.uniqueIndex(tableInfo.generatedColumns(), SimpleReference::column);

        Map<ColumnIdent, SimpleReference> defaultExpressionColumns =
            Maps.uniqueIndex(tableInfo.defaultExpressionColumns(), SimpleReference::column);
    ...
    private static Map<ColumnIdent, Integer> toPositionMap(List<SimpleReference> targetColumns) {
        if (targetColumns.isEmpty()) {
            return Collections.emptyMap();
        }
        Map<ColumnIdent, Integer> columnPositions = new HashMap<>(targetColumns.size(), 1);
        ListIterator<SimpleReference> it = targetColumns.listIterator();
        while (it.hasNext()) {
    ...
    private List<Symbol> symbolsFromTargetColumnPositionOrGeneratedExpression(Map<ColumnIdent, Integer> targetColumnMap,
        ...
        for (ColumnIdent column : columns) {
            ...
            if (position == null) {
                final Symbol symbol;
                SimpleReference reference = defaultExpressionColumns.get(column);
                if (reference != null) {
                    ...
                    if (generatedRef == null) {
                        SimpleReference columnRef = requireNonNull(targetTable.getReference(column),
