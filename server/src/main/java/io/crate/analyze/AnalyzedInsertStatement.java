...
public class AnalyzedInsertStatement implements AnalyzedStatement {
    ...
    AnalyzedInsertStatement(AnalyzedRelation subQueryRelation,
        ...
        Map<ColumnIdent, GeneratedReference> generatedColumns =
            Maps.uniqueIndex(tableInfo.generatedColumns(), ScopedRef::column);

        Map<ColumnIdent, ScopedRef> defaultExpressionColumns =
            Maps.uniqueIndex(tableInfo.defaultExpressionColumns(), ScopedRef::column);
    ...
    private static Map<ColumnIdent, Integer> toPositionMap(List<ScopedRef> targetColumns) {
        if (targetColumns.isEmpty()) {
            return Collections.emptyMap();
        }
        Map<ColumnIdent, Integer> columnPositions = HashMap.newHashMap(targetColumns.size());
        ListIterator<ScopedRef> it = targetColumns.listIterator();
        while (it.hasNext()) {
            ...
            if (position == null) {
                ...
                if (reference != null) {
                    ...
                    if (generatedRef == null) {
                        ScopedRef columnRef = requireNonNull(targetTable.getReference(column),
