...
public enum HttpErrorStatus {
    ...
    private static final Map<Integer, HttpErrorStatus> CODE_TO_STATUS;

    static {
        HttpErrorStatus[] values = values();
        Map<Integer, HttpErrorStatus> codeToStatus = new HashMap<>(values.length);
    ...
    public static HttpErrorStatus readFrom(StreamInput in) throws IOException {
        return HttpErrorStatus.valueOf(in.readString());
    }
    ...
    /**
     * Tries to convert a RestStatus comping from some old node to a HttpErrorStatus.
     * Falls back to UNHANDLED_SERVER_ERROR if the mapping is not possible.
     * Can be removed, once rolling upgrades from Version < 5.10.0 are not supported anymore.
     */
    public static HttpErrorStatus fromRestStatus(String restStatusName) {
        var restStatus = RestStatus.valueOf(restStatusName);
        var responseStatus = HttpResponseStatus.valueOf(restStatus.getStatus());
        var status = CODE_TO_STATUS.get(Integer.parseInt(String.format(Locale.ENGLISH, "%d0", responseStatus.code())));
        if (status != null) {
            return status;
        }
