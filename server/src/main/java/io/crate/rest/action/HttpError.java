...
public class HttpError {
    ...
    public static HttpError fromThrowable(Throwable throwable) {
        ...
        if (throwable instanceof InvalidRequest) {
            var err = (InvalidRequest) throwable;
            return new HttpError(err.httpResponseStatus(), err.errorCode(), err.getMessage(), throwable);
        }
        ...
        if (throwable instanceof MapperParsingException) {
            ...
            // Missinng GroupByOnArrayUnsupportedException, SchemaScopeException, TaskMissing, UnhandledServerException
        ...
        return new HttpError(httpErrorStatus, message, throwable);
    }
}
