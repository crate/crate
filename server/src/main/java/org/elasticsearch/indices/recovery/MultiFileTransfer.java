...
abstract class MultiFileTransfer<Request extends MultiFileTransfer.ChunkRequest> implements Closeable {
    ...
    protected MultiFileTransfer(Logger logger,
        ...
        this.processor = new AsyncIOProcessor<>(logger, maxConcurrentFileChunks) {
            @Override
    ...
    private void addItem(long requestSeqId, StoreFileMetadata md, Exception failure) {
        processor.put(
            new FileChunkResponseItem(requestSeqId, md, failure),
            e -> {
                assert e == null : e;
            }
        );
