...
public abstract class RemoteConnectionStrategy implements TransportConnectionListener, Closeable {
    ...
    static ConnectionProfile buildConnectionProfile(Settings nodeSettings,
        ...
        boolean compress = REMOTE_CONNECTION_COMPRESS.exists(connectionSettings)
            ? REMOTE_CONNECTION_COMPRESS.get(nodeSettings)
    ...
    static Set<String> getRemoteClusters(Settings settings) {
        return settings.getGroups("cluster.remote.").keySet();
    }
        ...
        synchronized (mutex) {
            ...
            if (closed) {
                ...
                if (listeners.size() >= maxPendingConnectionListeners) {
                    ...
                    listener.onFailure(new EsRejectedExecutionException("connect listener queue is full", runConnect));
                    return;
                } else {
