...
public class LoggingSslHandler extends LoggingByteToMessageDecoder implements ChannelOutboundHandler {

    private String name;
    private static final Logger LOGGER = LogManager.getLogger(LoggingSslHandler.class);

    private static final InternalLogger logger =
        InternalLoggerFactory.getInstance(LoggingSslHandler.class);
    ...
    private enum SslEngineType {
        ...
        JDK(false, MERGE_CUMULATOR) {
            @Override
            SSLEngineResult unwrap(LoggingSslHandler handler, ByteBuf in, int len, ByteBuf out) throws SSLException {
            ...
            @Override
            ByteBuf allocateWrapBuffer(LoggingSslHandler handler, ByteBufAllocator allocator,
            ...
            @Override
            int calculatePendingData(LoggingSslHandler handler, int guess) {
                return guess;
            }
        ...
        static LoggingSslHandler.SslEngineType forEngine(SSLEngine engine) {
            return JDK;
        }
        ...
        abstract SSLEngineResult unwrap(LoggingSslHandler handler, ByteBuf in, int len, ByteBuf out) throws SSLException;

        abstract int calculatePendingData(LoggingSslHandler handler, int guess);

        abstract boolean jdkCompatibilityMode(SSLEngine engine);

        abstract ByteBuf allocateWrapBuffer(LoggingSslHandler handler, ByteBufAllocator allocator,
                                            int pendingBytes, int numComponents);
    ...
    private final LoggingSslHandler.SslEngineType engineType;
    ...
    private final LoggingSslHandler.SslTasksRunner sslTaskRunnerForUnwrap = new LoggingSslHandler.SslTasksRunner(true);
    private final LoggingSslHandler.SslTasksRunner sslTaskRunner = new LoggingSslHandler.SslTasksRunner(false);

    private LoggingSslHandler.SslHandlerCoalescingBufferQueue pendingUnencryptedWrites;
    private Promise<Channel> handshakePromise = new LoggingSslHandler.LazyChannelPromise();
     ...
     */
    public LoggingSslHandler(SSLEngine engine, String name) {
        this(engine, false);
        this.name = name;
    }
     ...
     */
    public LoggingSslHandler(SSLEngine engine, boolean startTls) {
     ...
     */
    public LoggingSslHandler(SSLEngine engine, Executor delegatedTaskExecutor) {
     ...
     *                              {@link SSLEngine#getDelegatedTask()}.
     */
    public LoggingSslHandler(SSLEngine engine, boolean startTls, Executor delegatedTaskExecutor) {
        ...
        engineType = LoggingSslHandler.SslEngineType.forEngine(engine);
    ...
    public void handlerRemoved0(ChannelHandlerContext ctx) throws Exception {
        LOGGER.info("{}: in handlerRemoved0", name);
        try {
            ...
            if (!handshakePromise.isDone()) {
                LOGGER.info("{}: handshakePromise is not done in handlerRemoved0", name);
            ...
            if (!sslClosePromise.isDone()) {
                LOGGER.info("{}: sslClosePromise is not done in handlerRemoved0", name);
                if (cause == null) {
                    cause = new SSLHandshakeException("SslHandler removed before handshake completed");
                }
    ...
    public static boolean isEncrypted(ByteBuf buffer) {
        ...
        return getEncryptedPacketLength(buffer, buffer.readerIndex()) != NOT_ENCRYPTED;
    }
    ...
    private LoggingSslHandler.SslTasksRunner getTaskRunner(boolean inUnwrap) {
    ...
    private void executeDelegatedTask(LoggingSslHandler.SslTasksRunner task) {
        setState(STATE_PROCESS_TASK);
        try {
    ...
    private void setHandshakeFailure(ChannelHandlerContext ctx, Throwable cause, boolean closeInbound,
        ...
        try {
            ...
            if (handshakePromise.tryFailure(cause) || alwaysFlushAndClose) {
                handleHandshakeFailure(ctx, cause, notify);
            }
        } finally {
    ...
    public void handlerAdded(final ChannelHandlerContext ctx) throws Exception {
        ...
        pendingUnencryptedWrites = new LoggingSslHandler.SslHandlerCoalescingBufferQueue(channel, 16);
        ...
        final Future<?> timeoutFuture = ctx.executor().schedule(new Runnable() {
            ...
            public void run() {
                ...
                SSLException exception =
                    new SSLException("handshake timed out after " + handshakeTimeoutMillis + "ms");
                try {
    ...
    private final class SslHandlerCoalescingBufferQueue extends AbstractCoalescingBufferQueue {
        ...
        protected ByteBuf compose(ByteBufAllocator alloc, ByteBuf cumulation, ByteBuf next) {
            final int wrapDataSize = LoggingSslHandler.this.wrapDataSize;
